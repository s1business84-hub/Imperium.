...
export default function HomePage() {
  ...
  const featuresY = useTransform(scrollYProgress, [0.15, 0.5], [100, 0])
  const featuresOpacity = useTransform(scrollYProgress, [0.15, 0.25], [0, 1])
  
  const lampY = useTransform(scrollYProgress, [0.4, 0.7], [80, 0])
  const lampScale = useTransform(scrollYProgress, [0.5, 0.7], [0.9, 1])

  return (
    <div ref={containerRef}>
      <BackgroundLines className="relative" svgOptions={{ duration: 5 }}>
        <main className="mx-auto max-w-7xl px-4 py-4">
          {/* Hero: Welcome with Parallax */}
          <motion.div 
            style={{ y: heroY, opacity: heroOpacity, scale: heroScale }}
            className="mb-4 flex items-center justify-center text-center"
          >
        ...
        <motion.div style={{ y: featuresY, opacity: featuresOpacity }}>
          <FeaturesSection />
        </motion.div>

        {/* Start Exploring Button with Lamp Effect and Parallax */}
        <motion.div 
          style={{ y: lampY, scale: lampScale }}
          className="mt-12"
        >
